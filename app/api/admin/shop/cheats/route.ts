...
import { getShopCheatsForAdmin } from "@/lib/supabase/shop-queries";
...
export async function GET() {
  try {
    ...
    const rows = await getShopCheatsForAdmin(gate.scope);
    return NextResponse.json(rows, {
      headers: { "Cache-Control": "private, no-store" },
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error("[api/admin/shop/cheats]", message);
    const safeMessage =
